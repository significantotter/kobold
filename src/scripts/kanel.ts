	...
	preRenderHooks: [
		makeKyselyHook({
			...
			getKyselyItemMetadata: (d, selectorName, canInitialize, canMutate) => {
				...
				const tableName = toCamelCase(selectorName);
				return {
					tableInterfaceName: `${tableName}Table`,
					selectableName: tableName,
					insertableName: canInitialize ? `New${tableName}` : undefined,
					updatableName: canMutate ? `${tableName}Update` : undefined,
				};
			},
		}),
	],
