...
export async function parseItem(this: CompendiumEmbedParser, item: Item): Promise<KoboldEmbed> {
	...
	if (item.activate) descriptionLines.push(this.helpers.parseActivate(item.activate));
