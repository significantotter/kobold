...
type PasteBinImport = {
	sheet?: {
		 ...
		 */
		infoLists: { traits: Array<string>; languages: Array<string>; senses: Array<string> };
		defenses: {
			immunities: Array<
				{
					label: string;
					...
					amount?: number;
					appliesTo: Array<
						'damage' | 'effect' | 'condition' | 'critical-hit' | 'nonlethal'
					>;
					match: {
						all?: boolean;
					...
					automation: 'auto' | 'partial' | 'manual';
					source?: 'nethys' | 'pathbuilder' | 'wanderers-guide' | 'manual';
				} & { [x: string]: never }
			>;
			weaknesses: Array<
				{
	...
	actions?: Array<{
		actionCost: null | string;
		autoHeighten: boolean;
		baseLevel: null | number;
		description: string;
		id: number;
		name: string;
		sheetRecordId: null | number;
		...
		rolls: Array<
			...
			| ({
					...
					allowRollModifiers: boolean;
					type: 'save';
					saveRollType: null | string;
					saveTargetDC: null | string;
			  } & { [x: string]: never })
			| ({
					name: string;
					allowRollModifiers: boolean;
					...
					criticalSuccessText: null | string;
					criticalFailureText: null | string;
