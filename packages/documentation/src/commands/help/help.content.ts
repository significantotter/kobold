/**
 * Help command content - migrated from i18n/en/commands/help.ts
 * Contains all static content displayed by help subcommands
 */
...
export const helpEmbedLinks = {
	donate: `[Support Kobold's development through ko-fi](${helpLinks.donate})`,
	invite: `[Invite Kobold to a Discord Server](${helpLinks.invite})`,
	source: `[Follow Kobold's development on Github](${helpLinks.source})`,
	support: `[Join Kobold's Support Server on Discord](${helpLinks.support})`,
};

export interface HelpEmbedContent {
	title: string;
	description?: string;
...
export const helpContent = {
	...
	about: {
		...
		fields: [
			...
			},
			{
				name: 'Links',
				value:
					helpEmbedLinks.invite +
					'\n' +
					helpEmbedLinks.donate +
					'\n' +
					helpEmbedLinks.support +
					'\n' +
					helpEmbedLinks.source,
			},
		],
	} satisfies HelpEmbedContent,
	...
	commands: {
		...
		description: 'Use `/help <command-name>` for detailed help on a specific command.',
	} satisfies HelpEmbedContent,
	...
	attributesAndTags: {
		...
		fields: [
			...
			{
				name: 'Save',
				value: ['fortitude', 'reflex', 'will'].join('\n'),
				inline: true,
			},
			{
				name: 'Skill',
				value: [
					...
					'Survival',
					'Thievery',
					"(Custom skills/lores, e.g. 'Warfare_Lore')",
				].join('\n'),
				inline: true,
			},
			{
				name: 'Helpers',
				value: ['untrained', 'trained', 'expert', 'master', 'legendary'].join('\n'),
				inline: true,
			},
		],
	} satisfies HelpEmbedContent,
...
export const actionCreationWalkthroughs = {
	produceFlame: {
		title: 'Creating a Custom Action: Produce Flame',
		description:
			...
			'`/action-stage add-text action:Produce Flame roll-name:Critical Effect critical-success-text:The target takes {{[spellLevel]}}d4 persistent fire damage.`\n\n' +
	...
	fireball: {
		title: 'Creating a Custom Action: Fireball',
		description:
	...
	phantomPain: {
		title: 'Creating a Custom Action: Phantom Pain',
		description:
			...
			'`/action-stage add-text action:Phantom Pain roll-name:Effects failure-text:The target takes {{[spellLevel]}} ' +
			'persistent damage and becomes sickened 1. If they recover from sickened, the persistent damage ends. ' +
			'critical-failure-text:The target takes {{[spellLevel]}} persistent damage and becomes sickened 2. ' +
	...
	gunslingerPairedShots: {
		title: 'Creating a Custom Action: Gunslinger Paired Shots',
		description:
	...
	battleMedicine: {
		title: 'Creating a Custom Action: Battle Medicine',
		description:
