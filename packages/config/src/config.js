...
function parseEnvObjectArray(envVariable, params) {
    if (envVariable == '')
        return [];
    try {
        const initialResult = JSON.parse(envVariable);
        if (!_.isArray(initialResult))
            throw new Error('Not an array');
        const parsedResult = [];
        for (const envObject of initialResult) {
            const currentParsedObject = {};
            for (const [key, value] of Object.entries(params)) {
                if (!envObject[key])
                    throw new Error(`Missing key ${key} on object ${JSON.stringify(envObject)}`);
                else if (typeof envObject[key] !== value) {
                    throw new Error(`Key ${key} on object ${JSON.stringify(envObject)} is not of type ${value}`);
                }
                else {
...
function parseEnvObject(envVariable) {
    if (!envVariable)
        return undefined;
    return JSON.parse(envVariable);
}
function parseEnvNumber(envVariable) {
    if (!envVariable || isNaN(Number(envVariable)))
        return undefined;
    return Number(envVariable);
}
function parseEnvBoolean(envVariable) {
    if (!envVariable)
        return undefined;
...
export const Config = Object.freeze({
    ...
    jobs: {
        updateServerCount: {
            ...
            initialDelaySecs: parseEnvNumber(env.JOBS_UPDATE_SERVER_COUNT_RUN_INITIAL_DELAY_SECS ?? '') ?? 0,
        },
    },
    rateLimiting: {
        commands: {
    ...
    botSites: parseEnvObjectArray(env.BOT_SITES_0 ?? '', {
        name: 'string',
        enabled: 'boolean',
        url: 'string',
